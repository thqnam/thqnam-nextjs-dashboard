...
export async function sendSignUpEmail(email: string, name: string, token: string, id: string) {
  ...
  await transporter.sendMail({
    ...
    html: `
      ...
      <p>Or you can copy this link: <code>${Url}</code></p>
      <p>And paste it into your browser, if you do not like click it.</p>
      <p>But you must storage this recovery codes is: <code>${id}</code></p>
      <p>In your safe and secret place, for Reset Email address of this account.</p>
      <p>Because we only send this recovery code to you just one time.</p>
      <p>If you do not have an account in <a href="${process.env.APP_HOMEPAGE}">${process.env.APP_NAME}</a> App,</p>
      <p>Please ignore this email, sorry for bothering you<p>
    `,
  });
